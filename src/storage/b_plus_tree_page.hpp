...
/// B+ Tree page types
enum class BPTreePageType : uint8_t {
    INVALID = 0,
    INTERNAL = 1,
    LEAF = 2
};
...
public:
    ...
    [[nodiscard]] BPTreePageType page_type() const {
        return header()->page_type;
    }

    [[nodiscard]] uint32_t num_keys() const {
    ...
    [[nodiscard]] bool is_full() const {
        return num_keys() >= max_size();
    }

    /// Minimum keys for non-root nodes (max_size / 2)
    [[nodiscard]] uint32_t min_size() const {
        return max_size() / 2;
    }
...
protected:
    [[nodiscard]] const BPTreeHeader* header() const {
        return reinterpret_cast<const BPTreeHeader*>(
            page_->data() + config::kPageHeaderSize);
    }

    [[nodiscard]] BPTreeHeader* header_mut() {
        return reinterpret_cast<BPTreeHeader*>(
            page_->data() + config::kPageHeaderSize);
    }
...
public:
    ...
    [[nodiscard]] page_id_t child_at(uint32_t index) const {
        if (index > num_keys()) return INVALID_PAGE_ID;
        page_id_t child;
     ...
     *
     * @param index The index where to insert (key goes at index, child goes at index+1)
...
private:
    ...
    // Key i is at offset: sizeof(page_id_t) + i * (sizeof(key) + sizeof(page_id_t))
    // Child i is at offset: i * (sizeof(key) + sizeof(page_id_t)) for i > 0
    //                       0 for i = 0
    ...
    [[nodiscard]] const char* child_ptr(uint32_t index) const {
        ...
        return data_area() + sizeof(page_id_t) + (index - 1) * kPairSize + sizeof(BPTreeKey);
    }
...
public:
    ...
    void set_prev_leaf_id(page_id_t id) {
        std::memcpy(const_cast<char*>(data_area()) + sizeof(page_id_t), &id, sizeof(id));
        page_->set_dirty(true);
    }
    ...
    [[nodiscard]] BPTreeValue value_at(uint32_t index) const {
        if (index >= num_keys()) return BPTreeValue();
        BPTreeValue value;
     ...
     */
    [[nodiscard]] uint32_t find_key_index(BPTreeKey key, bool* found = nullptr) const {
        uint32_t n = num_keys();
