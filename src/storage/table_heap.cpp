...
TableHeap::TableHeap(std::shared_ptr<BufferPoolManager> buffer_pool, page_id_t first_page_id)
    : buffer_pool_(std::move(buffer_pool))
...
Status TableHeap::insert_tuple(const Tuple& tuple, RID* rid) {
    ...
    uint32_t max_tuple_size = static_cast<uint32_t>(config::kDefaultPageSize - config::kPageHeaderSize - TablePage::kSlotSize);
    ...
    TablePage table_page(page);
    auto slot_id = table_page.insert_record(tuple.data(), static_cast<uint16_t>(tuple_size));

    if (!slot_id.has_value()) {
        // This shouldn't happen if find_page_with_space worked correctly
        buffer_pool_->unpin_page(target_page_id, false);
        return Status::Internal("Failed to insert tuple into page with reported space");
    }
...
Status TableHeap::update_tuple(const Tuple& tuple, const RID& rid) {
    ...
    bool updated = table_page.update_record(
        rid.slot_id,
        tuple.data(),
    ...
    if (!status.ok()) {
        // This is a problem - we deleted the old tuple but couldn't insert the new one
        // In a real system, we'd use WAL to handle this atomically
        return Status::Internal("Failed to insert updated tuple: " + std::string(status.message()));
    }

    // Note: The RID may have changed. In a real system, we might need to
    // update any indexes pointing to this tuple.
    // For now, we return success but the caller should be aware the RID may have changed.
