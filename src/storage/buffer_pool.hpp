...
public:
     ...
     * page's LSN before the page reaches disk. This is a mechanism only; the
     * hook is empty until an owner installs one.
     */
    void set_wal_flush_hook(std::function<void(lsn_t)> hook);
...
private:
    ...
    std::function<void(lsn_t)> wal_flush_hook_;
