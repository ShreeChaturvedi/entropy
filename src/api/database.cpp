...
Status resolve_expr(Expression *expr, const Schema &schema,
    ...
    case ExpressionType::COLUMN_REF: {
        ...
        if (found < 0) {
            const std::string full = want_table.empty() ? want_col : want_table + "." + want_col;
    ...
    case ExpressionType::BINARY_OP: {
        ...
        Status s = resolve_expr(const_cast<Expression *>(b->left()), schema, col_tables);
        if (!s.ok()) {
            return s;
        }
        ...
        // still yields NULL.
        auto bad = [](TypeId t) { return t == TypeId::VARCHAR || t == TypeId::BOOLEAN; };
        if (bad(b->left()->result_type()) || bad(b->right()->result_type())) {
            return Status::InvalidArgument("arithmetic operator requires numeric operands");
        }
        return Status::Ok();
    }
    case ExpressionType::COMPARISON: {
        ...
        Status s = resolve_expr(const_cast<Expression *>(c->left()), schema, col_tables);
        if (!s.ok()) {
            return s;
        }
        return resolve_expr(const_cast<Expression *>(c->right()), schema, col_tables);
    }
    case ExpressionType::LOGICAL: {
        ...
        Status s = resolve_expr(const_cast<Expression *>(l->left()), schema, col_tables);
        if (!s.ok()) {
            return s;
        }
        return resolve_expr(const_cast<Expression *>(l->right()), schema, col_tables);
    }
    case ExpressionType::IS_NULL: {
        ...
        return resolve_expr(const_cast<Expression *>(n->operand()), schema, col_tables);
    }
    }
    return Status::Ok();
}
...
public:
    ...
    [[nodiscard]] uint64_t epoch_for(const std::shared_ptr<SessionTable> &table) const {
...
public:
    ...
    explicit DatabaseImpl(const std::string &path, const DatabaseOptions &options,
        ...
        if (options.page_size != config::kDefaultPageSize) {
            open_error_ = "Unsupported page_size " + std::to_string(options.page_size) +
                          ": this build supports only " + std::to_string(config::kDefaultPageSize) +
        ...
        if (injected_disk != nullptr) {
            ...
            disk_manager_ = std::make_shared<FileDiskManager>(path, options.create_if_missing,
        ...
        buffer_pool_ = std::make_shared<BufferPoolManager>(options.buffer_pool_size, disk_manager_);
        ...
        if (options.enable_wal) {
            wal_manager_ = std::make_shared<WALManager>(path + config::kWALFileExtension);
        ...
        if (wal_manager_) {
            ...
            if (!recovered.ok()) {
                open_error_ = "Crash recovery failed: " + std::string(recovered.message());
                LOG_ERROR("{}", open_error_);
                return;
            }
        ...
        catalog_ = std::make_shared<Catalog>(buffer_pool_, path + config::kCatalogFileExtension);
        ...
        txn_manager_->set_table_resolver([catalog = catalog_](oid_t table_oid) -> TableStore * {
        ...
        txn_manager_->set_index_resolver([catalog = catalog_](oid_t index_oid) -> Index * {
        ...
        index_selector_ = std::make_unique<IndexSelector>(catalog_, statistics_, cost_model_);
        optimizer_ =
            std::make_unique<Optimizer>(catalog_, statistics_, cost_model_, index_selector_.get());
    ...
    Result execute(std::string_view sql) {
        if (!is_open_) {
            return Result(Status::InvalidArgument(open_error_.empty() ? "Database is not open"
                                                                      : open_error_));
        }
        ...
        case StatementType::CREATE_TABLE:
            return execute_create_table(dynamic_cast<CreateTableStatement *>(stmt.get()));
        case StatementType::DROP_TABLE:
            return execute_drop_table(dynamic_cast<DropTableStatement *>(stmt.get()));
        case StatementType::CREATE_INDEX:
            return execute_create_index(dynamic_cast<CreateIndexStatement *>(stmt.get()));
        case StatementType::DROP_INDEX:
            return execute_drop_index(dynamic_cast<DropIndexStatement *>(stmt.get()));
        ...
        if (session.bound && session.txn == nullptr) {
            return Result(
                Status::Aborted("Current transaction is aborted; statements are rejected until "
                                "rollback"));
        }
        ...
        ExecutorContext exec_ctx{txn, txn_manager_.get(), lock_manager_.get(), version_store_.get(),
                                 catalog_.get()};
        ...
        Result result = [&]() -> Result {
            try {
                switch (stmt->type()) {
                case StatementType::SELECT:
                    return execute_select(dynamic_cast<SelectStatement *>(stmt.get()), &exec_ctx);
                case StatementType::INSERT:
                    return execute_insert(dynamic_cast<InsertStatement *>(stmt.get()), &exec_ctx);
                case StatementType::UPDATE:
                    return execute_update(dynamic_cast<UpdateStatement *>(stmt.get()), &exec_ctx);
                case StatementType::DELETE_STMT:
                    return execute_delete(dynamic_cast<DeleteStatement *>(stmt.get()), &exec_ctx);
                default:
    ...
    Result finalize_query(std::unique_ptr<Executor> executor, const Schema *schema,
                          SelectStatement *stmt, ExecutorContext *ctx) {
        if (!stmt->order_by.empty()) {
            ...
            for (const auto &item : stmt->order_by) {
                ...
                if (idx < 0) {
                    return Result(
                        Status::InvalidArgument("ORDER BY column not found: " + item.column_name));
                }
        ...
        if (stmt->limit.has_value() || stmt->offset.has_value()) {
            executor = std::make_unique<LimitExecutor>(ctx, std::move(executor), stmt->limit,
    ...
    Result collect_result(Executor *executor, const Schema *output_schema) {
        ...
        while (auto tuple = executor->next()) {
            ...
            for (size_t i = 0; i < output_schema->column_count(); i++) {
                TupleValue tv = tuple->get_value(*output_schema, static_cast<uint32_t>(i));
    ...
    std::unique_ptr<Executor> build_plan_executor(const PlanNode *node, ExecutorContext *ctx) {
        switch (node->type()) {
        case PlanNodeType::SEQ_SCAN: {
            ...
            std::unique_ptr<Expression> pred = s->predicate() ? s->predicate()->clone() : nullptr;
            return std::make_unique<SeqScanExecutor>(ctx, info->table_heap, s->output_schema(),
                                                     std::move(pred));
        }
        case PlanNodeType::INDEX_SCAN: {
            ...
            case IndexScanPlanNode::ScanType::POINT_LOOKUP:
                return std::make_unique<IndexScanExecutor>(
                    ctx, index_info->index.get(), info->table_heap.get(), s->output_schema(),
        ...
        case PlanNodeType::NESTED_LOOP_JOIN: {
            ...
            std::unique_ptr<Expression> cond = j->condition() ? j->condition()->clone() : nullptr;
            return std::make_unique<NestedLoopJoinExecutor>(
                ctx, std::move(left), std::move(right), node->children()[0]->output_schema(),
        ...
        case PlanNodeType::HASH_JOIN: {
            ...
            return std::make_unique<HashJoinExecutor>(
                ctx, std::move(left), std::move(right), node->children()[0]->output_schema(),
                node->children()[1]->output_schema(), j->output_schema(), j->left_key_index(),
                j->right_key_index(), to_parser_join_type(j->join_type()));
        }
        default:
            return nullptr;
        }
    ...
    std::unique_ptr<Executor> build_scan_executor(oid_t table_oid, const Schema *schema,
                                                  std::unique_ptr<Expression> filter,
                                                  ExecutorContext *ctx, Status *status) {
    ...
    Result execute_select(SelectStatement *stmt, ExecutorContext *exec_ctx) {
        ...
        if (!stmt->joins.empty() || !stmt->group_by.empty() || has_aggregate || has_computed) {
    ...
    Result execute_select_simple(SelectStatement *stmt, ExecutorContext *exec_ctx) {
        ...
        if (!stmt->order_by.empty()) {
            ...
            executor = std::make_unique<SortExecutor>(exec_ctx, std::move(executor), input_schema,
                                                      std::move(sort_keys));
        }
        ...
        if (ctx.select_all) {
            ...
            auto proj = std::make_unique<ProjectionExecutor>(exec_ctx, std::move(executor),
                                                             input_schema, ctx.column_indices);
            output_schema = &proj->output_schema();
            executor = std::move(proj);
        }

        if (stmt->limit.has_value() || stmt->offset.has_value()) {
            executor = std::make_unique<LimitExecutor>(exec_ctx, std::move(executor), stmt->limit,
                                                       stmt->offset.value_or(0));
        }
    ...
    Result execute_select_query(SelectStatement *stmt, ExecutorContext *exec_ctx) {
        ...
        if (!has_joins && stmt->where_clause) {
            ...
            Status s = resolve_expr(stmt->where_clause.get(), *relations[0].schema, base_tables);
            if (!s.ok()) {
                return Result(s);
            }
        ...
        for (size_t ji = 0; ji < stmt->joins.size(); ++ji) {
            ...
            if (jc.condition) {
                ...
                if (auto *cmp = dynamic_cast<ComparisonExpression *>(jc.condition.get());
        ...
        std::unique_ptr<PlanNode> plan;
        Status planned = optimizer_->optimize(std::move(relations), std::move(joins), &plan);
        if (!planned.ok()) {
        ...
        if (has_joins && stmt->where_clause) {
            ...
            executor = std::make_unique<FilterExecutor>(exec_ctx, std::move(executor),
                                                        std::move(stmt->where_clause), cur_schema);
        }
        ...
        if (has_aggregate || !stmt->group_by.empty()) {
            return build_aggregation(stmt, std::move(executor), cur_schema, cur_tables, exec_ctx);
        }

        // Projection (computed/aliased expressions, or a plain column list). A lone
        // `*` keeps the combined schema unchanged.
        const bool single_star = stmt->columns.size() == 1 && stmt->columns[0].is_star;
        if (!single_star) {
            ...
            for (auto &col : stmt->columns) {
                if (col.is_star) {
                    ...
                    for (size_t i = 0; i < cur_schema->column_count(); ++i) {
                        auto ref =
                            std::make_unique<ColumnRefExpression>(cur_schema->column(i).name());
                ...
                if (col.expression) {
                    ...
                    auto ref =
                        std::make_unique<ColumnRefExpression>(col.column_name, col.table_alias);
                ...
                std::string name = !col.alias.empty()
                                       ? col.alias
                                       : (col.column_name.empty() ? "expr" : col.column_name);
                out_cols.emplace_back(name, type);
                exprs.push_back(std::move(e));
            }

            auto proj = std::make_unique<ProjectionExecutor>(
                exec_ctx, std::move(executor), cur_schema, std::move(exprs), std::move(out_cols));
    ...
    Result build_aggregation(SelectStatement *stmt, std::unique_ptr<Executor> child,
        ...
        for (const auto &name : stmt->group_by) {
            ...
            if (idx < 0) {
                return Result(Status::InvalidArgument("GROUP BY column not found: " + name));
            }
        ...
        for (auto &col : stmt->columns) {
            if (col.is_star) {
                return Result(Status::NotSupported("SELECT * cannot be combined with aggregation"));
            }
            if (col.agg_func != AggregateFunc::NONE) {
                size_t arg_index = 0;
                if (col.agg_func != AggregateFunc::COUNT_STAR) {
                    ...
                    if (ref == nullptr) {
                        return Result(
                            Status::NotSupported("aggregate argument must be a column reference"));
                    }
                    ...
                    if ((col.agg_func == AggregateFunc::SUM ||
                        ...
                        return Result(
                            Status::InvalidArgument("SUM/AVG requires a numeric argument"));
                    }
                    arg_index = ref->column_index();
                }
                std::string name = col.alias.empty() ? "agg" : col.alias;
                aggregates.emplace_back(to_exec_aggregate(col.agg_func), arg_index, name);
        ...
        auto agg_exec = std::make_unique<AggregationExecutor>(
            exec_ctx, std::move(child), input_schema, group_by_indices, std::move(aggregates));
        ...
        for (size_t c = 0; c < stmt->columns.size(); ++c) {
            ...
            const size_t out_index = select_group_pos[c].has_value() ? *select_group_pos[c]
                                                                     : group_count + agg_counter++;
            auto ref = std::make_unique<ColumnRefExpression>(agg_schema->column(out_index).name());
        ...
        auto proj = std::make_unique<ProjectionExecutor>(exec_ctx, std::move(agg_exec), agg_schema,
                                                         std::move(exprs), std::move(out_cols));
    ...
    Result execute_insert(InsertStatement *stmt, ExecutorContext *exec_ctx) {
        ...
        InsertExecutor insert(exec_ctx, ctx.table_info->table_heap, std::move(tuples),
                              ctx.table_info->oid);
        insert.init();
        (void)insert.next();
    ...
    Result execute_update(UpdateStatement *stmt, ExecutorContext *exec_ctx) {
        ...
        auto child =
            std::make_unique<SeqScanExecutor>(exec_ctx, ctx.table_info->table_heap,
                                              &ctx.table_info->schema, std::move(ctx.predicate));

        UpdateExecutor update(exec_ctx, std::move(child), ctx.table_info->table_heap,
                              &ctx.table_info->schema, std::move(ctx.column_indices),
                              std::move(ctx.values), ctx.table_info->oid);
        update.init();
        (void)update.next();
    ...
    Result execute_create_index(CreateIndexStatement *stmt) {
        ...
        if (stmt->columns.size() != 1) {
            return Result(Status::NotSupported("Only single-column indexes are supported"));
        }
        Status status = catalog_->create_index(stmt->index_name, stmt->table_name, stmt->columns[0],
                                               stmt->is_unique);
        if (!status.ok()) {
            return Result(status);
        }
    ...
    Result execute_drop_table(DropTableStatement *stmt) {
        ...
        if (auto info = catalog_->get_table_shared(stmt->table_name); info && info->table_heap) {
        ...
        Status status = catalog_->drop_table(stmt->table_name, /*deallocate_heap_pages=*/false);
        if (!status.ok()) {
            return Result(status);
        }
        ...
        if (wal_manager_) {
            RecoveryManager recovery(buffer_pool_, wal_manager_, disk_manager_);
            Status checkpointed = recovery.create_checkpoint(txn_manager_->get_active_txn_ids(),
    ...
    // Render one PlanNode (and its children) into plan_lines. Indentation grows
    // with tree depth so join plans read as a nested tree. Strings for scan
    // kinds keep the legacy "Index Scan" / "Sequential Scan" needles that
    // integration tests and shell users already match.
    void format_plan_node(const PlanNode *node, int depth,
                          std::vector<std::string> *plan_lines) const {
        if (node == nullptr || plan_lines == nullptr) {
            return;
        }
        const std::string indent(static_cast<size_t>(depth) * 2u, ' ');
        std::string line = indent + "-> ";

        switch (node->type()) {
        case PlanNodeType::SEQ_SCAN: {
            const auto *s = static_cast<const SeqScanPlanNode *>(node);
            const TableInfo *table = catalog_->get_table(s->table_oid());
            line += "Sequential Scan";
            if (table != nullptr) {
                line += " on " + table->name;
            }
            line += "  (cost=" + std::to_string(node->cost()) + ")";
            plan_lines->push_back(line);
            if (s->predicate() != nullptr) {
                plan_lines->push_back(indent + "   Filter: (predicate)");
            }
            break;
        }
        case PlanNodeType::INDEX_SCAN: {
        ...
        case PlanNodeType::NESTED_LOOP_JOIN: {
            const auto *j = static_cast<const NestedLoopJoinPlanNode *>(node);
            line += "Nested Loop Join  (cost=" + std::to_string(node->cost()) + ")";
            plan_lines->push_back(line);
            if (j->condition() != nullptr) {
                plan_lines->push_back(indent + "   Join Filter: (predicate)");
            }
            break;
        }
        case PlanNodeType::HASH_JOIN: {
            const auto *j = static_cast<const HashJoinPlanNode *>(node);
            line += "Hash Join  (cost=" + std::to_string(node->cost()) + ")";
            plan_lines->push_back(line);
            plan_lines->push_back(indent + "   Hash Cond: left[" +
                                  std::to_string(j->left_key_index()) + "] = right[" +
                                  std::to_string(j->right_key_index()) + "]");
            break;
        }
        case PlanNodeType::FILTER:
            plan_lines->push_back(indent + "-> Filter  (cost=" + std::to_string(node->cost()) +
                                  ")");
            plan_lines->push_back(indent + "   Filter: (predicate)");
            break;
        case PlanNodeType::PROJECTION:
            plan_lines->push_back(indent + "-> Projection  (cost=" + std::to_string(node->cost()) +
                                  ")");
            break;
        case PlanNodeType::SORT:
            plan_lines->push_back(indent + "-> Sort  (cost=" + std::to_string(node->cost()) + ")");
            break;
        case PlanNodeType::LIMIT: {
            const auto *l = static_cast<const LimitPlanNode *>(node);
            std::string lim = indent + "-> Limit";
            if (l->limit().has_value()) {
                lim += ": " + std::to_string(*l->limit());
            }
            lim += "  (cost=" + std::to_string(node->cost()) + ")";
            plan_lines->push_back(std::move(lim));
            break;
        }
        case PlanNodeType::AGGREGATION:
            plan_lines->push_back(indent + "-> Aggregate  (cost=" + std::to_string(node->cost()) +
                                  ")");
            break;
        case PlanNodeType::INSERT:
        case PlanNodeType::UPDATE:
        case PlanNodeType::DELETE:
            plan_lines->push_back(indent + "-> Modify  (cost=" + std::to_string(node->cost()) +
                                  ")");
            break;
        }

        for (const auto &child : node->children()) {
            format_plan_node(child.get(), depth + 1, plan_lines);
        }
    }

    // Build the scan/join PlanNode tree using the same optimizer path as
    // execute_select_simple / execute_select_query. owned_schemas holds combined
    // join schemas for the duration of EXPLAIN.
    Status plan_select_for_explain(SelectStatement *select, std::unique_ptr<PlanNode> *plan,
                                   std::vector<std::unique_ptr<Schema>> *owned_schemas,
                                   std::vector<std::shared_ptr<TableInfo>> *pinned) {
        if (select->joins.empty()) {
            // Single-table: binder resolves the table and WHERE, then the optimizer
            // picks seq scan vs index scan exactly as execute_select_simple does.
            BoundSelectContext ctx;
            Status status = binder_->bind_select(select, &ctx);
            if (!status.ok()) {
                return status;
            }
            pinned->push_back(ctx.table_info);
            std::vector<ScanSpec> relations;
            relations.push_back(
                ScanSpec{ctx.table_info->oid, &ctx.table_info->schema, std::move(ctx.predicate)});
            return optimizer_->optimize(std::move(relations), {}, plan);
        }

        // Multi-table: mirror the relation/join construction in execute_select_query
        // so EXPLAIN shows the same HashJoin vs NestedLoopJoin choice execute uses.
        std::vector<TableRef> table_refs;
        table_refs.push_back(select->table);
        ...
        std::vector<Column> combined_cols;
        std::vector<std::string> combined_tables;
        std::vector<size_t> relation_widths;
        relation_widths.reserve(table_refs.size());
        for (const auto &ref : table_refs) {
            std::shared_ptr<TableInfo> info = catalog_->get_table_shared(ref.table_name);
            if (info == nullptr) {
            ...
            const std::string alias = ref.alias.empty() ? ref.table_name : ref.alias;
            const size_t width = info->schema.column_count();
            relation_widths.push_back(width);
            relations.push_back(ScanSpec{info->oid, &info->schema, nullptr});
        ...
        // Push WHERE into the base scan only when there are no joins (handled
        // above). With joins, residual WHERE is applied above the join at execute
        // time and is not part of the optimizer PlanNode tree today.

        const size_t base_width = relation_widths.front();
        std::vector<JoinSpec> joins;
        std::vector<Column> running_cols(
            combined_cols.begin(), combined_cols.begin() + static_cast<std::ptrdiff_t>(base_width));
        std::vector<std::string> running_tables(combined_tables.begin(),
                                                combined_tables.begin() +
                                                    static_cast<std::ptrdiff_t>(base_width));
        size_t col_cursor = base_width;

        for (size_t ji = 0; ji < select->joins.size(); ++ji) {
            JoinClause &jc = select->joins[ji];
            const size_t left_width = running_cols.size();
            const size_t right_width = relation_widths[ji + 1];
        ...
        return optimizer_->optimize(std::move(relations), std::move(joins), plan);
    }
    ...
    Result execute_explain(ExplainStatement *stmt) {
        ...
        // Plan through the same optimizer path execute uses, then walk the tree.
        std::vector<std::unique_ptr<Schema>> owned_schemas;
        std::vector<std::shared_ptr<TableInfo>> pinned;
        std::unique_ptr<PlanNode> plan;
        Status status = plan_select_for_explain(select, &plan, &owned_schemas, &pinned);
        if (!status.ok()) {
            return Result(status);
        }
        if (plan == nullptr) {
            return Result(Status::Internal("EXPLAIN produced an empty plan"));
        }

        std::vector<std::string> plan_lines;
        plan_lines.push_back("Query Plan:");
        format_plan_node(plan.get(), 0, &plan_lines);

        // Sort and Limit are applied as executor wrappers on the execute path, not
        // as PlanNodes, so surface them from the statement so EXPLAIN still names
        // the post-scan operators SELECT will run.
        ...
        const size_t estimated_rows = cost_model_->estimate_cardinality(plan.get());
    ...
    static TupleValue parsed_value_to_tuple_value(const InsertValue &pv, TypeId target_type) {
    ...
    Status begin_transaction() {
        ...
        if (it != sessions_->bindings.end()) {
            ...
            if (it->second.txn != nullptr &&
                txn_manager_->is_active_transaction(it->second.txn_id, it->second.txn)) {
            ...
            // database.
            {
                ...
                for (auto &entry : sessions_->bindings) {
                    ...
                    if (binding.txn != nullptr &&
                        txn_manager_->is_active_transaction(binding.txn_id, binding.txn)) {
...
Database::Database(const std::string &path, std::shared_ptr<DiskManager> disk_manager,
                   const DatabaseOptions &options)
    : impl_(std::make_unique<DatabaseImpl>(path, options, std::move(disk_manager))) {}
