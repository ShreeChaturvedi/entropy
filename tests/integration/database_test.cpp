...
protected:
    void SetUp() override { temp_file_ = std::make_unique<test::TempFile>("db_test_"); }
...
TEST_F(DatabaseTest, OpenFailureLeavesDatabaseClosed) {
    ...
    const std::string bad_path = temp_file_->string() + "/missing_parent/db.entropy";
...
TEST_F(DatabaseTest, SelectWithWhere) {
    ...
    auto result = db.execute("CREATE TABLE users (id INTEGER, name VARCHAR(100), age INTEGER)");
    EXPECT_TRUE(result.ok()) << result.status().to_string();
...
TEST_F(DatabaseTest, InsertMultipleRows) {
    ...
    auto result = db.execute("CREATE TABLE users (id INTEGER, name VARCHAR(100))");
    EXPECT_TRUE(result.ok()) << result.status().to_string();
    result = db.execute("INSERT INTO users VALUES (1, 'Alice'), (2, 'Bob'), (3, 'Charlie')");
    EXPECT_TRUE(result.ok()) << result.status().to_string();
...
TEST_F(DatabaseTest, SelectNarrowNumericTypes) {
    ...
    auto result = db.execute("CREATE TABLE nums (s SMALLINT, t TINYINT, f FLOAT)");
    EXPECT_TRUE(result.ok()) << result.status().to_string();
...
TEST_F(DatabaseTest, IndexScanResolvesIndexByOidEndToEnd) {
    ...
    ASSERT_TRUE(db.execute("CREATE TABLE t (a INTEGER, pad INTEGER, c INTEGER)").ok());
    ...
    const auto &row = result.rows()[0];
    ASSERT_EQ(row.size(), 3u);
    EXPECT_EQ(row[0].as_int32(), kV) << "wrong index resolved: OID was cast to a column_id";
...
TEST_F(DatabaseTest, TwoTableInnerJoinReturnsCorrectRows) {
    ...
    ASSERT_TRUE(db.execute("CREATE TABLE dept (did INTEGER, name VARCHAR)").ok());
    ASSERT_TRUE(db.execute("INSERT INTO emp VALUES (1,10),(2,10),(3,20),(4,30)").ok());
    ASSERT_TRUE(db.execute("INSERT INTO dept VALUES (10,'Eng'),(20,'Sales')").ok());

    // emp row 4 (dept 30) has no matching department and must be dropped by the
    // inner join.
    auto result = db.execute("SELECT emp.id AS id, dept.name AS name "
    ...
    const std::vector<std::pair<int, std::string>> want = {{1, "Eng"}, {2, "Eng"}, {3, "Sales"}};
    EXPECT_EQ(got, want);
}
...
TEST_F(DatabaseTest, GroupByAggregatesAreCorrect) {
    ...
    ASSERT_TRUE(db.execute("INSERT INTO emp VALUES (10,100),(10,200),(20,300),(20,50),"
                           "(20,120)")
                    .ok());

    auto result = db.execute("SELECT dept, COUNT(*) AS c, SUM(sal) AS s, MIN(sal) AS mn, "
...
TEST_F(DatabaseTest, ComputedAndAliasedProjection) {
    ...
    auto result = db.execute("SELECT id AS emp_id, sal + 10 AS bonus, sal * 2 AS doubled FROM t "
                             "ORDER BY emp_id");
    ASSERT_TRUE(result.ok()) << result.status().to_string();
    ASSERT_EQ(result.row_count(), 2u);
...
TEST_F(DatabaseTest, IllTypedExpressionReturnsErrorNotCrash) {
    Database db(temp_file_->string());
    ASSERT_TRUE(db.execute("CREATE TABLE t (id INTEGER, name VARCHAR(100))").ok());
...
TEST_F(DatabaseTest, IllTypedDmlReturnsErrorNotCrash) {
    Database db(temp_file_->string());
    ASSERT_TRUE(db.execute("CREATE TABLE emp (id INTEGER, name VARCHAR(50), sal INTEGER)").ok());
    ASSERT_TRUE(db.execute("INSERT INTO emp VALUES (1,'Alice',100),(2,'Bob',200)").ok());
...
TEST_F(DatabaseTest, OptimizerChoosesIndexVsSeqScan) {
    ...
    EXPECT_TRUE(explain_contains("EXPLAIN SELECT * FROM t WHERE id = 5", "Index Scan"));
    // No predicate -> sequential scan.
    EXPECT_TRUE(explain_contains("EXPLAIN SELECT * FROM t", "Sequential Scan"));
...
// #104: EXPLAIN must walk the optimizer PlanNode tree so multi-table plans
// (hash vs nested-loop join) match what execute actually runs.
TEST_F(DatabaseTest, ExplainJoinWalksPlanNodeTree) {
    Database db(temp_file_->string());
    ASSERT_TRUE(db.execute("CREATE TABLE emp (id INTEGER, dept INTEGER)").ok());
    ASSERT_TRUE(db.execute("CREATE TABLE dept (did INTEGER, name VARCHAR)").ok());
    ASSERT_TRUE(db.execute("INSERT INTO emp VALUES (1,10),(2,10),(3,20)").ok());
    ASSERT_TRUE(db.execute("INSERT INTO dept VALUES (10,'Eng'),(20,'Sales')").ok());

    auto explain = db.execute("EXPLAIN SELECT emp.id, dept.name FROM emp JOIN dept "
    ...
    bool saw_cost = false;
    for (const auto &r : explain.rows()) {
        const std::string &line = r[0].as_string();
        if (line.find("Hash Join") != std::string::npos ||
            line.find("Nested Loop Join") != std::string::npos) {
            saw_join = true;
        }
        if (line.find("Sequential Scan") != std::string::npos ||
            line.find("Index Scan") != std::string::npos) {
            saw_scan = true;
        }
    ...
    EXPECT_TRUE(saw_join) << "EXPLAIN join plan missing join node";
    EXPECT_TRUE(saw_scan) << "EXPLAIN join plan missing scan children";
